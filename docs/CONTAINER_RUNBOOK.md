...
- It writes **two** files: `backend/.env` (read directly by the app through its bind mount) and
  the repo-root `.env` (read by Compose for `${VAR}` interpolation, so Postgres and Redis get the
  generated passwords instead of the `CHANGEME-run-app-setup` placeholders). It does **not** read
  `.env.example` — both templates are embedded in the module.
- **Both targets must be on a bind mount.** The long-running services mount only `backend/`,
  `scripts/` and `config/`, so a setup run through one of them writes the root `.env` into the
  container's own filesystem, prints "created", and loses it to `--rm`. That is why setup has its
  own service: it is the only one that mounts the repository root. It sits behind a `setup`
  profile so `docker compose up` never starts it. The prebuilt-image files
  (`docker-compose.public.yml`, `.demo.yml`) have no source checkout to mount over `/app` —
  doing so would hide the application code baked into the image — so they mount the install
  directory at `/hostenv` and redirect setup with `FIREMASTER_APP_ENV` / `FIREMASTER_ROOT_ENV`.
- `main.py` startup errors now point at this command instead of a manual bcrypt one-liner.
- **Run it once:** `docker compose run --rm --no-deps setup`.
...
| Backend crashes: `JWT_SECRET_KEY must be set…` | No valid `.env` yet | run the setup command (Change 4): `docker compose run --rm --no-deps setup` |
...
| Postgres or Redis rejects the password, or a service logs `CHANGEME-run-app-setup` | Setup never ran, so the compose placeholder defaults are still in effect | run `docker compose run --rm --no-deps setup` — it generates random credentials and writes both `.env` files |
